...
function listener(details) {
    ...
    if(details.url.includes("https://pos.toshin.com/KKS/KKS1/Page/Design/KozaInfo.aspx?KozaCode=")){
        ...
        // details.responseHeaders["Content-Type"] = "text/html; charset=utf8";
        return {};
    }
    ...
    filter.onstop = event => {
        ...
        if (str.indexOf("A477C046-2D9B-40CF-94C0-427C9C99E580://nagase.com/openwith") != -1) {
            ...
            str = str.replace("</head>","<script>$.get(\"https://pos.toshin.com/KKS/KKS1/Page/Design/KozaInfo.aspx?KozaCode="+kozaid+"&Refresh=1\", function(data, status){const result = data.match(/<td nowrap=\\\"nowrap\\\" align=\\\"Center\\\" valign=\\\"Middle\\\">.+?<\\\/td><td>.+?<\\\/td>/g);var kozaList = {};for(var i=0; i < result.length; i++){var kozainfo = result[i].substring(51,result[i].length-5).split(\"</td><td>\");var kozaNum = kozainfo[0];var kozaName = kozainfo[1];kozaList[kozaNum.replace(/ /g,'')] = kozaName;}var title2List = document.getElementsByClassName(\"tit_02\");for(var i=0; i < title2List.length; i++){if(kozaList[title2List[i].innerText.replace(/ /g,'')]){title2List[i].innerText = title2List[i].innerText.replace(/[Ａ-Ｚａ-ｚ０-９！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝]/g, function (s) {return String.fromCharCode(s.charCodeAt(0) - 0xfee0);}).replace(/[‐－―]/g, \"-\").replace(/[～〜]/g, \"~\").replace(/　/g, \" \") + \" - \" + kozaList[title2List[i].innerText.replace(/ /g,'')].replace(/[Ａ-Ｚａ-ｚ０-９！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝]/g, function (s) {return String.fromCharCode(s.charCodeAt(0) - 0xfee0);}).replace(/[‐－―]/g, \"-\").replace(/[～〜]/g, \"~\").replace(/　/g, \" \");}}});</script></head>");
