...
function listener(details) {
    ...
    if (details.url.endsWith("PlayerSelector.aspx")) {
        ...
        gettingItem.then((res) => {
            ...
            console.log("[Player Config Update] 視聴ログ送信は" + (debug_do_not_send_watch_log ? "無効" : "有効") + "です。");
        });
    }
    ...
    filter.onstop = event => {
        ...
        if (str.indexOf("A477C046-2D9B-40CF-94C0-427C9C99E580://nagase.com/openwith") != -1) {
            ...
            console.log("[Player] 現在、視聴ログ送信は" + (debug_do_not_send_watch_log ? "無効" : "有効") + "です。");
